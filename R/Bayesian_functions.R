...
#'
#' \code{inferBayesianGenotype} infers an subject's genotype applying a Bayesian framework with Dirichlet prior 
...
#' 
#' @return   A table of alleles denoting the genotype of the subject with the log10
...
#' data(germline_ighv)
#' data(novel_df)
#' inferBayesianGenotype(sample_db, find_unmutated = TRUE, germline_db = germline_ighv,
#'               novel_df = novel_df)
#' 
...
#' 
#' @export
inferBayesianGenotype <- function(clip_db,v_call = 'V_CALL', find_unmutated = TRUE,
  ...
  ALLELES = COUNTS = KH=KD=KT=KQ=Kdiff=NOTE = rep("", length(GENE))
  TOTAL = sapply(gene_groups, length)
  genotype = cbind(GENE, ALLELES, COUNTS, TOTAL, NOTE,KH,KD,KT,KQ,Kdiff)
  ...
  for (g in GENE){
    ...
    genotype[genotype[, "GENE"] == g, "Kdiff"] =probs[5];
    #     }
...
get_probabilites_with_priors <- function(X,alpha_dirichlet=c(0.5,0.5,0.5,0.5)*2,epsilon=0.01,priors=c(0.5,0.5,0.33,0.33,0.33,0.25,0.25,0.25,0.25)){
  ## Hypotheses 
