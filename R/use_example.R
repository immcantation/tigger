...
subjects = unique(hiv_annot$SUBJECT)
to_ignore <- c(453548,702909,729593,911594,211578,966971,588311,961360,911594,729593,NA)
subjects = setdiff(subjects, to_ignore)

new_calls = list()
for (ii in 1:length(subjects){
  dat = subset(hiv_annot, SUBJECT %in% subjects[ii] & FUNCTIONAL == "TRUE")
  ...
  if (correct_calls) {
    ...
    new_calls[subjects[ii]] = reassignAlleles(v_calls, v_sequences, genotype_db)
  }
  cat("Completed subject", subjects[ii],".\n")
}
