...
#'   \item \code{note}: Comments regarding the inference.
    ...
    out_list <- foreach(idx=iterators::icount(length(allele_groups))) %dopar% {
        ...
        for (mut_min in rev(mut_mins)) {
            ...
            pos_muts <- pos_db %>%
                ...
                dplyr::summarise(POS_MUT_RATE = mean(!!rlang::sym("MUTATED"))*unique(!!rlang::sym("PASS")) ) %>%
                dplyr::ungroup()
            ...
            db_y_subset_mm$MUT_COUNT_MINUS_SUBSTRING <- db_y_subset_mm[[seq]] %>%
...
plotNovel <- function(data, novel_row, v_call="v_call", j_call="j_call",
    ...
    # MAKE THE THIRD PLOT
    p3 <- ggplot(db_subset, aes(x=!!rlang::sym(junction_length),
                                fill=!!rlang::sym("J_GENE"))) +
        geom_bar(width=0.9) +
...
#' any differences.
#'
#' @param    samples        vector of strings representing aligned sequences
#' @param    germlines      vector of strings representing aligned sequences
...
#' @param    allele_calls  vector of strings representing Ig allele calls for
...
#'
#' @param    allele_calls   vector of strings representing Ig allele calls,
...
#' provided sequence.
#'
#' @param    sequence     starting nucleotide sequence.
#' @param    positions    numeric vector of positions which to be changed.
#' @param    nucleotides  character vector of nucleotides to which to change the
#'                        positions.
#'
...
#'
#' @param    fasta_file       fasta-formatted file of immunoglobulin sequences.
...
#'                            uppercase.
#'
#' @return   Named vector of strings representing Ig alleles.
#'
...
#'
#' @return   A named vector of strings representing Ig alleles.
#'
...
#'
#' @param    allele_calls  vector of strings representing IGHV allele names.
#'
#' @return   Vector of strings representing updated IGHV allele names.
#'
#' @note
#' IMGT has removed \code{IGHV2-5*10} and \code{IGHV2-5*07} as it has determined they
...
#' Sort allele names
#'
#' \code{sortAlleles} returns a sorted vector of strings representing Ig allele
...
#'
#' @param    allele_calls  vector of strings representing Ig allele names.
#' @param    method        string defining the method to use when sorting alleles.
#'                         If \code{"name"} then sort in lexicographic order. If
#'                         \code{"position"} then sort by position in the locus, as
#'                         determined by the final two numbers in the gene name.
#' @return   A sorted vector of strings representing Ig allele names.
#'
...
#'                 also \code{group}.
#' @param   min_n  minimum number of observations to sample from each group. A group with
