...
```{r}
i_novel_df <- tigger(sample_db, germline_ighv, 
                     fields=NULL,
                     nproc=1, max.iter=Inf)
names(i_novel_df)
```
...
The function `plotTigger` will take the output of `tigger` and generate figures 
for the newly identified alleles. The figures are sotre in a list and can be printed 
independently or arranged in panels.
In this example, `tigger` identified `r  length(i_novel_df$new_germlines)` new germline in the input dataset.
...
# Note:
# Polymorphisms:
# - The first "1" refers to the groups created with the `fields` in `tigger`.
