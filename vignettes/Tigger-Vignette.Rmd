...
in your own fasta file using `readGermlineDb`.
...
made by `findNovelAlleles` using the function `plotTigger`.
...
sequences, the use may use the function `genotypeFasta`.
...
```{r, eval=TRUE, warning=FALSE}
# Determine the fraction of sequences that were ambigious before correction
sum(grepl(",",sample_db$V_CALL))/nrow(sample_db)
# Determine the fraction of sequences that were ambigious after correction
sum(grepl(",",sample_db$V_CALL_GENOTYPED))/nrow(sample_db)
...
# Find the fraction that contained original calls to non-genotype alleles 
sum(sample_db$V_CALL %in% not_in_genotype)/nrow(sample_db)
