...
each. (If any of the aforementioned options of `runTigger` were set to `FALSE`,
the corresponding elements of `sample_output` will simply be empty.)
...
germline_example = c(germline_example, novel_sequences)
```
...
`plotNovelJunctions()` can be used to view the distribution of J genes and
junctions.
...
Additionally, genes representing fewere than `0.1%` of sequences are, by
...
genotype_sequences = genotypeFasta(sample_output$genotype, germline_example)
```
